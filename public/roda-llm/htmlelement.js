...
${p}`:p,a=a?`${a}
${b}`:b;let k=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(b,s,!0),this.lexer.state.top=k,n.length===0)break;let y=s.at(-1);if(y?.type==="code")break;if(y?.type==="blockquote"){let v=y,x=v.raw+`
`+n.join(`
...
`))!==-1;){let h=p.slice(0,l);p=p.slice(l+2),v(h)}};e.addEventListener=(l,h)=>((o[l]||=[]).push(h),e),e.removeEventListener=(l,h)=>(o[l]=(o[l]||[]).filter(f=>f!==h),e),e.close=()=>{e.readyState!==e.CLOSED&&(e.readyState=e.CLOSED,c?.abort(),c=null)},e.active=()=>e.readyState===e.OPEN||e.readyState===e.CONNECTING,c=new AbortController;let u=c.signal;return fetch(n,{method:i,headers:k,body:b,signal:u,credentials:s?"include":"same-origin"}).then(async l=>{if(!l.ok)throw new Error(await l.text()||`${l.status}`);if(e.readyState===e.CLOSED)return;e.readyState=e.OPEN;let h=l.body.getReader(),f=new TextDecoder;for(;;){let{value:w,done:E}=await h.read();if(E)break;p+=f.decode(w,{stream:!0}),x()}e.close()}).catch(l=>{if(e.readyState===e.CLOSED||l.name==="AbortError"){e.close();return}e.close();let h=typeof MessageEvent=="function"?new MessageEvent("error",{data:l.message||String(l)}):{type:"error",data:l.message||String(l),message:l.message};for(let f of o.error||[])f(h)}),e}function Xe(t={}){let e=Object.create(null),{path:n,headers:r}=t,a=["onContent","onToolCall","onToolReturn","onGoodbye","onError"],s=null;return e.onContent=t.onContent||(i=>{}),e.onToolCall=t.onToolCall||(i=>{}),e.onToolReturn=t.onToolReturn||(i=>{}),e.onGoodbye=t.onGoodbye||(i=>{}),e.onError=t.onError||(i=>{}),e.attach=i=>{if(s)return e;let o=We({url:n,headers:r,body:{q:i}});for(let c of a)o.addEventListener(c,p=>{e[c]?.(JSON.parse(p.data)),(c==="onGoodbye"||c==="onError")&&e.close()});return o.addEventListener("error",c=>{e.onError({error:c.data}),e.close()}),s=o,e},e.close=()=>{s&&(s.close(),s=null)},e.active=()=>!!s,e}function Ye(){let t=Object.create(null);return t.active=new Map,t.completed=new Map,t.onToolCall=e=>{t.active.set(e.id,e.name)},t.onToolReturn=e=>{t.active.delete(e.id),t.completed.set(e.name,(t.completed.get(e.name)||0)+1)},t}var Ke={dot:"M12 12h.01",chevron:"M6 9l6 6 6-6",list:"M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01",person:"M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2M12 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8",speech:"M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z",tool:"M14.7 6.3a4 4 0 0 0 5 5l-9.4 9.4a2.1 2.1 0 0 1-3-3zM6 3l3 3M3 6l3 3",return:"M9 14l-4-4 4-4M5 10h9a5 5 0 0 1 0 10h-2",clock:"M12 22a10 10 0 1 0 0-20 10 10 0 0 0 0 20zM12 6v6l4 2",warn:"M12 9v4M12 17h.01M10.3 3.9 1.8 18a2 2 0 0 0 1.7 3h17a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0z"},Kt=200,Vt=6;function Ve(t){let e=Ke[t]||Ke.dot,n=document.createElementNS("http://www.w3.org/2000/svg","svg");n.setAttribute("class","ac-icon"),n.setAttribute("viewBox","0 0 24 24"),n.setAttribute("fill","none"),n.setAttribute("stroke","currentColor"),n.setAttribute("stroke-width","2"),n.setAttribute("stroke-linecap","round"),n.setAttribute("stroke-linejoin","round"),n.setAttribute("aria-hidden","true");let r=document.createElementNS("http://www.w3.org/2000/svg","path");return r.setAttribute("d",e),n.append(r),n}function Qe({root:t,host:e}){let n=Object.create(null),r=t.querySelector(".ac-panels"),a=t.querySelector(".ac-activity"),s=t.querySelector(".ac-panel-view"),i=s.querySelector(".ac-panel-title"),o=s.querySelector(".ac-panel-body"),c=s.querySelector(".ac-panel-note"),p=!1;n.items=[],n.open=null;let b=()=>typeof s.showPopover=="function";n.set=m=>{n.items=m||[],r.innerHTML="",n.items.forEach((d,A)=>{A&&r.append(k());let R=document.createElement("button");R.type="button",R.className="ac-panel",R.dataset.panel=d.label,R.setAttribute("part","panel"),R.setAttribute("aria-expanded","false");let g=document.createElement("span");if(g.className="ac-panel-label",g.textContent=d.label,R.append(g),d.badge){let T=document.createElement("span");T.className="ac-panel-badge",T.textContent=d.badge,R.append(T)}R.addEventListener("pointerenter",()=>n.show(d.label)),R.addEventListener("focus",()=>n.show(d.label)),R.addEventListener("click",T=>{T.preventDefault(),T.stopPropagation(),n.toggle(d.label)}),d.aside&&R.append(...Array.isArray(d.aside)?d.aside:[d.aside]),r.append(R)}),n.open&&!n.items.some(d=>d.label===n.open)&&n.close()};function k(){let m=document.createElement("span");return m.className="ac-panel-divider",m.setAttribute("part","panel-divider"),m.setAttribute("aria-hidden","true"),m.textContent="|",m}function y(m){return r.querySelector(`.ac-panel[data-panel="${m}"]`)}n.refresh=()=>{for(let m of r.querySelectorAll(".ac-panel")){let d=m.dataset.panel===n.open;m.classList.toggle("is-open",d),m.setAttribute("aria-expanded",d?"true":"false")}},n.toggle=m=>{n.open===m?n.close():n.show(m)},n.show=async m=>{let d=n.items.find(A=>A.label===m);if(d){if(n.open!==m&&(n.open=m,n.refresh(),e.setAttribute("panel",m),i.textContent=m,o.innerHTML="",c.hidden=!0,x(m),l()),d.render){c.hidden=!0,o.innerHTML="",d.render(o);return}if(d.content){c.hidden=!0,o.innerHTML="";for(let A of d.content)o.append(A);return}o.childNodes.length||(c.textContent="Loading\u2026",c.hidden=!1);try{d.rows=await d.load()}catch{o.childNodes.length||(c.textContent="That could not be loaded.",c.hidden=!1);return}if(n.open===m){if(c.hidden=!0,d.html){o.innerHTML=d.rows||"";return}n.draw(d.rows)}}},n.close=()=>{n.open&&(n.open=null,n.refresh(),e.removeAttribute("panel"),h(),o.innerHTML="")};function v(m){let d=e.querySelectorAll("agent-panel[data-side]");for(let A of d)if(A.getAttribute("label")===m)return A.dataset.side;return null}function x(m){let d=e.getBoundingClientRect(),A=v(m);if(A==="right"&&window.innerWidth>=720){s.style.top=`${Math.round(d.top)}px`,s.style.left=`${Math.round(d.right)}px`,s.style.width=`${Math.round(d.width)}px`,s.style.height=`${Math.round(d.height)}px`,s.style.bottom="auto";return}if(A==="right"){s.style.top=`${Math.round(d.top)}px`,s.style.left=`${Math.round(d.left)}px`,s.style.width=`${Math.round(d.width)}px`,s.style.height=`${Math.round(d.height)}px`,s.style.bottom="auto";return}let g=(y(m)||a).getBoundingClientRect(),T=a.getBoundingClientRect(),_=Math.min(Math.max(Math.round(T.width/3),Kt),Math.round(T.width)),I=Math.min(Math.max(8,Math.round(g.left)),Math.max(8,window.innerWidth-_-8));s.style.left=`${I}px`,s.style.width=`${_}px`,s.style.top="auto",s.style.bottom=`${Math.round(window.innerHeight-g.top+Vt)}px`}function u(){if(p)return;p=!0,document.addEventListener("pointerdown",d=>{if(!n.open)return;let A=d.composedPath();!A.includes(s)&&!A.includes(r)&&n.close()},!0),document.addEventListener("keydown",d=>{d.key==="Escape"&&n.close()});let m=s.querySelector(".ac-panel-close");m&&m.addEventListener("click",()=>n.close())}typeof ResizeObserver=="function"&&new ResizeObserver(()=>{n.open&&x(n.open)}).observe(e),typeof window<"u"&&window.addEventListener("resize",()=>{n.open&&x(n.open)});function l(){u(),b()?s.matches(":popover-open")||s.showPopover():s.classList.add("is-open")}function h(){b()&&s.matches(":popover-open")&&s.hidePopover(),s.classList.remove("is-open")}n.draw=m=>{o.innerHTML="";for(let d of f(m||[])){let A=d.rows.map(w);if(!d.name){A.forEach(I=>o.append(I));continue}let R=document.createElement("details");R.className="ac-panel-group",R.setAttribute("part","panel-group"),R.open=d.last;let g=document.createElement("summary");g.className="ac-panel-group-summary",g.setAttribute("part","panel-group-summary"),g.append(Ve("chevron"));let T=document.createElement("span");T.textContent=d.name,g.append(T);let _=document.createElement("span");_.className="ac-panel-group-count",_.textContent=`${d.rows.length}`,g.append(_),R.append(g),A.forEach(I=>R.append(I)),R.addEventListener("toggle",()=>{if(R.open)for(let I of o.querySelectorAll(".ac-panel-group[open]"))I!==R&&(I.open=!1)}),o.append(R)}o.childNodes.length||(c.textContent="Nothing here yet.",c.hidden=!1)};function f(m){let d=[];for(let A of m){let R=A.group||"",g=d[d.length-1];(!g||g.name!==R)&&(g={name:R,rows:[]},d.push(g)),g.rows.push(A)}return d.length&&(d[d.length-1].last=!0),d}function w(m){let d=document.createElement("div");d.className="ac-panel-row",m.tone&&d.classList.add(`is-${m.tone}`),m.icon&&(d.dataset.icon=m.icon),d.setAttribute("part","panel-row");let A=m.params||m.pairs||[],R=!!(A.length||m.result),g=document.createElement("details");g.className="ac-panel-row-details",g.open=!R;let T=document.createElement("summary");T.className="ac-panel-row-label";let _=document.createElement("span");_.className="ac-panel-row-what";let I=document.createElement("span");I.className="ac-panel-row-head";let F=document.createElement("span");if(F.className="ac-panel-row-kind",F.textContent=m.kind||"",m.running&&(F.dataset.running="true"),I.append(F),m.title){let C=document.createElement("span");C.className="ac-panel-row-title",C.textContent=m.title,I.append(C)}if(_.append(I),m.subject)_.append(E(m.subject,"ac-panel-row-subject"));else if(m.detail){let C=document.createElement("span");C.className="ac-panel-row-detail",C.textContent=m.detail,_.append(C)}if(T.append(Ve(m.icon),_),m.duration){let C=document.createElement("span");C.className="ac-panel-row-duration",C.textContent=m.duration,T.append(C)}if(g.append(T),R){let C=document.createElement("div");C.className="ac-panel-row-body";for(let[N,P]of A)P&&C.append(E([N,P],"ac-panel-row-pair"));if(m.result&&m.result[1]){let N=E(m.result,"ac-panel-row-pair ac-panel-row-result");m.tone&&N.classList.add(`is-${m.tone}`),C.append(N)}g.append(C)}return d.append(g),d}function E([m,d],A){let R=document.createElement("span");R.className=A;let g=document.createElement("span");if(g.className="ac-panel-row-key",g.textContent=m,d){let T=document.createElement("span");T.className="ac-panel-row-value",T.textContent=d,R.append(g,T)}return R}return n}function Je(t={}){let e=Object.create(null),n=t.host,r=t.root,a=r.querySelector(".ac-answer"),s=r.querySelector(".ac-form"),i=r.querySelector(".ac-reset"),o=r.querySelector(".ac-expand"),c=Ye(),p=Qe({root:r,host:n}),b=r.querySelector('slot[name="panels"]'),k=()=>(b?.assignedElements()||[]).filter(g=>g.localName==="agent-panel").map(g=>g.panel()),y=()=>[...k(),...t.panels||[]].filter(Boolean);b&&b.addEventListener("slotchange",()=>p.set(y()));let v={data:{...t.labels||{},fallback:{call:"Running {name} tool\u2026",return:"Tool {name} finished"}},for(g){let{data:T}=this;return T[g]||T["*"]||T.default||T.fallback},slot(g,T){for(let _ of n.children){let I=_.getAttribute("slot");if(I===`tool.${g}.${T}`||I===`tool.*.${T}`)return _}},text(g,T,_){let I=T==="return"?"return":"call",F=this.slot(g,I);if(F)return F.textContent;let C=this.for(g);switch(T){case"return":return typeof C=="string"?_.count?C:void 0:C.return;default:return typeof C=="string"?C:C.call}},icon(g,T){let _=T==="return"?"return-icon":"call-icon",I=this.slot(g,_);if(I)return I;let F=this.for(g);return typeof F=="object"&&F&&F[_]?F[_]:w.default(T)},substitute(g,T){return g.replace(/\{(\w+)(?:\.(\w+))?\}/g,(_,I,F)=>I==="arguments"?F?T.arguments?.[F]??"":"":T[I]??"")},resolve(g,T){let _=this.text(g.name,T,g);return typeof _=="function"&&(_=_(g)),typeof _=="string"&&(_=this.substitute(_,g)),{text:_,icon:this.icon(g.name,T)}},active(g){return this.resolve(g,"active")},done(g){return this.resolve(g,"return")}},x={el:r.querySelector(".ac-status"),thinking:"Thinking\u2026",render({text:g,icon:T,thinking:_}){let{el:I}=this;if(I.replaceChildren(),T&&I.append(T.cloneNode(!0)),g!=null){let F=document.createElement("span");F.textContent=g,_&&(F.className="is-thinking"),I.append(F)}},follow(){let g=[...c.active.keys()].pop();if(g!=null){this.render(v.active({id:g,name:c.active.get(g)}));return}if(d){this.render({text:this.thinking,thinking:!0});return}this.render({})}},u={el:r.querySelector(".ac-placeholder"),last:r.querySelector(".ac-last-message"),show(){let{el:g}=this;g.hidden=!1,this.last&&(this.last.hidden=!0)},hide(){let{el:g}=this;g.hidden=!0,this.last&&(this.last.hidden=!0)},restore(g){this.last&&(t.renderer.render(g,this.last),this.el.classList.add("is-restored"),this.el.hidden=!0,this.last.hidden=!1)},forget(){this.last&&(this.el.classList.remove("is-restored"),this.el.hidden=!1,this.last.hidden=!0,this.last.innerHTML="")}},l={el:r.querySelector(".ac-loading"),show(){this.el&&(this.el.hidden=!1)},hide(){this.el&&(this.el.hidden=!0)}};u.hide(),l.show();let h=g=>{l.hide(),n.dispatchEvent(new CustomEvent("describe",{detail:g})),!A&&(g?.last_message?u.restore(g.last_message):u.show())},f=async()=>{try{let g=await t.http.describe();if(!g.ok)throw new Error(String(g.status));h(await g.json())}catch{h(void 0)}},w={el:r,default(g){let{el:T}=this,_=T.querySelector(g==="return"?".ac-icon-return":".ac-icon-call");return _?_.content.cloneNode(!0):void 0}},E={el:r.querySelector(".ac-input"),get value(){return this.el.value},clear(){this.el.value=""},busy(g){this.el.readOnly=g,r.classList.toggle("is-busy",g),n.toggleAttribute("busy",g),g?(this.resting=this.el.placeholder,this.el.placeholder="Working on your request..."):this.resting&&(this.el.placeholder=this.resting,this.resting=null)},focus(){this.el.focus({preventScroll:!0})}},m="",d=!1,A=!1,R=Xe({path:t.path,headers:t.headers,onContent(g){d=!1,x.follow(),u.hide(),m+=g.text,t.renderer.render(m,a)},onToolCall(g){c.onToolCall(g),d=!1,x.follow()},onToolReturn(g){c.onToolReturn(g),d=!0,x.follow()},onGoodbye(g){c.active.clear(),d=!1,x.follow(),E.busy(!1),m.trim()===""&&g.answer&&t.renderer.render(g.answer,a),f()},onError(g){c.active.clear(),d=!1,x.follow(),E.busy(!1),u.hide(),a.classList.add("is-error");let T="Something went wrong. Please try again.";try{T=g.error||T}catch{}a.textContent=T,f()}});return e.talk=g=>{R.active()||(A=!0,c.active.clear(),c.completed.clear(),m="",a.innerHTML="",a.classList.remove("is-error"),u.hide(),E.busy(!0),d=!0,x.follow(),R.attach(g),E.clear(),E.focus())},e.restore=g=>{l.hide(),g?.last_message?u.restore(g.last_message):u.show()},e.reset=async()=>{R.close();try{await t.http.destroy()}catch{}a.innerHTML="",a.classList.remove("is-error"),u.forget(),u.show(),c.active.clear(),c.completed.clear(),d=!1,A=!1,x.follow(),E.busy(!1),E.focus(),f()},e.focus=()=>E.focus(),e.clear=()=>{a.innerHTML="",a.classList.remove("is-error")},s.addEventListener("submit",g=>{g.preventDefault();let T=E.value.trim();T&&e.talk(T)}),i.addEventListener("click",e.reset),o.addEventListener("click",()=>{let g=r.classList.toggle("is-expanded");n.toggleAttribute("expanded",g),o.setAttribute("aria-expanded",g?"true":"false"),o.title=g?"Collapse chat":"Expand chat",E.focus()}),x.follow(),f(),p.set(y()),e.setPanels=g=>{t.panels=g||[],p.set(y())},e}var Qt=`
<style>
  :host {
    display: block;
