...
`,t=t.substring(y.length+1),c=!0),!c){let d=this.rules.other.nextBulletRegex(x),h=this.rules.other.hrRegex(x),m=this.rules.other.fencesBeginRegex(x),S=this.rules.other.headingBeginRegex(x),k=this.rules.other.htmlBeginRegex(x);for(;t;){let g=t.split(`
`,1)[0],A;if(y=g,this.options.pedantic?(y=y.replace(this.rules.other.listReplaceNesting,"  "),A=y):A=y.replace(this.rules.other.tabCharGlobal,"    "),m.test(y)||S.test(y)||k.test(y)||d.test(y)||h.test(y))break;if(A.search(this.rules.other.nonSpaceChar)>=x||!y.trim())b+=`
...
`))!==-1;){let d=p.slice(0,l);p=p.slice(l+2),v(d)}};e.addEventListener=(l,d)=>((o[l]||=[]).push(d),e),e.removeEventListener=(l,d)=>(o[l]=(o[l]||[]).filter(h=>h!==d),e),e.close=()=>{e.readyState!==e.CLOSED&&(e.readyState=e.CLOSED,c?.abort(),c=null)},e.active=()=>e.readyState===e.OPEN||e.readyState===e.CONNECTING,c=new AbortController;let u=c.signal;return fetch(n,{method:i,headers:w,body:b,signal:u,credentials:s?"include":"same-origin"}).then(async l=>{if(!l.ok)throw new Error(await l.text()||`${l.status}`);if(e.readyState===e.CLOSED)return;e.readyState=e.OPEN;let d=l.body.getReader(),h=new TextDecoder;for(;;){let{value:m,done:S}=await d.read();if(S)break;p+=h.decode(m,{stream:!0}),x()}e.close()}).catch(l=>{if(e.readyState===e.CLOSED||l.name==="AbortError"){e.close();return}e.close();let d=typeof MessageEvent=="function"?new MessageEvent("error",{data:l.message||String(l)}):{type:"error",data:l.message||String(l),message:l.message};for(let h of o.error||[])h(d)}),e}function Xe(t={}){let e=Object.create(null),{path:n,headers:r}=t,a=["onContent","onToolCall","onToolReturn","onGoodbye","onError"],s=null;return e.onContent=t.onContent||(i=>{}),e.onToolCall=t.onToolCall||(i=>{}),e.onToolReturn=t.onToolReturn||(i=>{}),e.onGoodbye=t.onGoodbye||(i=>{}),e.onError=t.onError||(i=>{}),e.attach=i=>{if(s)return e;let o=We({url:n,headers:r,body:{q:i}});for(let c of a)o.addEventListener(c,p=>{e[c]?.(JSON.parse(p.data)),(c==="onGoodbye"||c==="onError")&&e.close()});return o.addEventListener("error",c=>{e.onError({error:c.data}),e.close()}),s=o,e},e.close=()=>{s&&(s.close(),s=null)},e.active=()=>!!s,e}function Ye(){let t=Object.create(null);return t.active=new Map,t.completed=new Map,t.onToolCall=e=>{t.active.set(e.id,e.name)},t.onToolReturn=e=>{t.active.delete(e.id),t.completed.set(e.name,(t.completed.get(e.name)||0)+1)},t}function Ke({root:t,host:e}){let n=Object.create(null),r=t.querySelector(".ac-panels"),a=t.querySelector(".ac-activity"),s=t.querySelector(".ac-panel-view"),i=s.querySelector(".ac-panel-title"),o=s.querySelector(".ac-panel-body"),c=s.querySelector(".ac-panel-note"),p=!1;n.items=[],n.open=null,n.set=k=>{n.items=k||[],queueMicrotask(()=>{let g=n.items.find(A=>A.open);g&&!n.open&&n.show(g.label)}),r.innerHTML="",n.items.forEach((g,A)=>{A&&r.append(b());let T=document.createElement("button");T.type="button",T.className="ac-panel",T.dataset.panel=g.label,T.setAttribute("part","panel"),T.setAttribute("aria-expanded","false");let f=document.createElement("span");if(f.className="ac-panel-label",f.textContent=g.label,T.append(f),g.badge){let R=document.createElement("span");R.className="ac-panel-badge",R.textContent=g.badge,T.append(R)}y(g.label)==="hover"&&T.addEventListener("pointerenter",()=>{n.open||n.show(g.label)}),T.addEventListener("click",R=>{R.preventDefault(),R.stopPropagation(),n.toggle(g.label)}),g.aside&&T.append(...Array.isArray(g.aside)?g.aside:[g.aside]),r.append(T)}),n.open&&!n.items.some(g=>g.label===n.open)&&n.close()};function b(){let k=document.createElement("span");return k.className="ac-panel-divider",k.setAttribute("part","panel-divider"),k.setAttribute("aria-hidden","true"),k.textContent="|",k}function w(k){return r.querySelector(`.ac-panel[data-panel="${k}"]`)}n.refresh=()=>{for(let k of r.querySelectorAll(".ac-panel")){let g=k.dataset.panel===n.open;k.classList.toggle("is-open",g),k.setAttribute("aria-expanded",g?"true":"false")}},n.toggle=k=>{n.open===k?n.close():n.show(k)},n.show=async k=>{let g=n.items.find(T=>T.label===k);if(!g)return;if(n.open!==k&&(n.open=k,n.refresh(),e.setAttribute("panel",k),i.textContent=k,c.hidden=!0,x(k),m()),n.follow=()=>{let T=n.items.find(f=>f.follow);T&&n.open!==T.label&&n.show(T.label)},g.render){c.hidden=!0,o.innerHTML="",g.render(o);return}if(g.content){c.hidden=!0,o.innerHTML="";for(let T of g.content)o.append(T);return}o.childNodes.length||(c.textContent="Loading\u2026",c.hidden=!1);let A;try{A=await g.load()}catch{o.childNodes.length||(c.textContent="That could not be loaded.",c.hidden=!1);return}n.open===k&&(c.hidden=!0,o.innerHTML=A||"")},n.close=()=>{n.open&&(n.open=null,n.refresh(),e.removeAttribute("panel"),S(),o.innerHTML="")};function y(k){let g=e.querySelectorAll("agent-panel[trigger]");for(let A of g)if(A.getAttribute("label")===k)return A.getAttribute("trigger");return null}function v(k){let g=e.querySelectorAll("agent-panel[appears-on]");for(let A of g)if(A.getAttribute("label")===k)return A.getAttribute("appears-on");return null}function x(k){if(v(k)==="right"){s.classList.add("is-side"),s.style.top=s.style.left=s.style.width=s.style.height=s.style.bottom="";return}s.classList.remove("is-side");let g=e.getBoundingClientRect(),T=(w(k)||a).getBoundingClientRect(),f=a.getBoundingClientRect(),R=Math.min(Math.max(Math.round(f.width/3),200),Math.round(f.width)),$=Math.min(Math.max(8,Math.round(T.left-g.left)),Math.max(8,Math.round(g.width-R-8)));s.style.left=`${$}px`,s.style.width=`${R}px`,s.style.top="auto",s.style.bottom=`${Math.round(g.bottom-T.top+6)}px`}function u(){if(p)return;p=!0,document.addEventListener("pointerdown",g=>{if(!n.open)return;let A=g.composedPath();!A.includes(s)&&!A.includes(r)&&!A.includes(e)&&n.close()},!0),document.addEventListener("keydown",g=>{g.key==="Escape"&&(t.querySelector("dialog[open]")||n.close())});let k=s.querySelector(".ac-panel-close");k&&k.addEventListener("click",()=>n.close())}typeof ResizeObserver=="function"&&new ResizeObserver(()=>{n.open&&x(n.open)}).observe(e),typeof window<"u"&&window.addEventListener("resize",()=>{n.open&&x(n.open)});let l=null;function d(){l&&clearTimeout(l),l=null}function h(){d(),l=setTimeout(()=>{l=null,n.close()},240)}function m(){u(),s.classList.add("is-open")}function S(){s.classList.remove("is-open")}if(typeof e<"u"&&typeof setInterval=="function"){let k=!1,g=()=>{if(!n.open)return;let A=n.open;n.open=null,n.show(A)};setInterval(()=>{let A=e.hasAttribute("busy");(A||k)&&g(),k=A},1200)}return n}function Ve(t={}){let e=Object.create(null),n=t.host,r=t.root,a=r.querySelector(".ac-answer"),s=r.querySelector(".ac-form"),i=r.querySelector(".ac-reset"),o=r.querySelector(".ac-expand"),c=Ye(),p=Ke({root:r,host:n}),b=r.querySelector('slot[name="panels"]'),w=()=>(b?.assignedElements()||[]).filter(f=>f.localName==="agent-panel").map(f=>f.panel()),y=()=>[...w(),...t.panels||[]].filter(Boolean);b&&b.addEventListener("slotchange",()=>p.set(y()));let v={data:{...t.labels||{},fallback:{call:"Running {name} tool\u2026",return:"Tool {name} finished"}},for(f){let{data:R}=this;return R[f]||R["*"]||R.default||R.fallback},slot(f,R){for(let $ of n.children){let L=$.getAttribute("slot");if(L===`tool.${f}.${R}`||L===`tool.*.${R}`)return $}},text(f,R,$){let L=R==="return"?"return":"call",F=this.slot(f,L);if(F)return F.textContent;let D=this.for(f);switch(R){case"return":return typeof D=="string"?$.count?D:void 0:D.return;default:return typeof D=="string"?D:D.call}},icon(f,R){let $=R==="return"?"return-icon":"call-icon",L=this.slot(f,$);if(L)return L;let F=this.for(f);return typeof F=="object"&&F&&F[$]?F[$]:m.default(R)},substitute(f,R){return f.replace(/\{(\w+)(?:\.(\w+))?\}/g,($,L,F)=>L==="arguments"?F?R.arguments?.[F]??"":"":R[L]??"")},resolve(f,R){let $=this.text(f.name,R,f);return typeof $=="function"&&($=$(f)),typeof $=="string"&&($=this.substitute($,f)),{text:$,icon:this.icon(f.name,R)}},active(f){return this.resolve(f,"active")},done(f){return this.resolve(f,"return")}},x={el:r.querySelector(".ac-status"),thinking:"Thinking\u2026",render({text:f,icon:R,thinking:$}){let{el:L}=this;if(L.replaceChildren(),R&&L.append(R.cloneNode(!0)),f!=null){let F=document.createElement("span");F.textContent=f,$&&(F.className="is-thinking"),L.append(F)}},follow(){let f=[...c.active.keys()].pop();if(f!=null){this.render(v.active({id:f,name:c.active.get(f)}));return}if(g){this.render({text:this.thinking,thinking:!0});return}this.render({})}},u={el:r.querySelector(".ac-placeholder"),last:r.querySelector(".ac-last-message"),show(){let{el:f}=this;f.hidden=!1,this.last&&(this.last.hidden=!0)},hide(){let{el:f}=this;f.hidden=!0,this.last&&(this.last.hidden=!0)},restore(f){this.last&&(t.renderer.render(f,this.last),this.el.classList.add("is-restored"),this.el.hidden=!0,this.last.hidden=!1)},forget(){this.last&&(this.el.classList.remove("is-restored"),this.el.hidden=!1,this.last.hidden=!0,this.last.innerHTML="")}},l={el:r.querySelector(".ac-loading"),show(){this.el&&(this.el.hidden=!1)},hide(){this.el&&(this.el.hidden=!0)}};u.hide(),l.show();let d=f=>{l.hide(),n.dispatchEvent(new CustomEvent("describe",{detail:f})),!A&&(f?.last_message?u.restore(f.last_message):u.show())},h=async()=>{try{let f=await t.http.describe();if(!f.ok)throw new Error(String(f.status));d(await f.json())}catch{d(void 0)}},m={el:r,default(f){let{el:R}=this,$=R.querySelector(f==="return"?".ac-icon-return":".ac-icon-call");return $?$.content.cloneNode(!0):void 0}},S={el:r.querySelector(".ac-input"),get value(){return this.el.value},clear(){this.el.value=""},busy(f){this.el.readOnly=f,r.classList.toggle("is-busy",f),n.toggleAttribute("busy",f),f&&p.follow(),f?(this.resting=this.el.placeholder,this.el.placeholder="Working on your request..."):this.resting&&(this.el.placeholder=this.resting,this.resting=null)},focus(){this.el.focus({preventScroll:!0})}},k="",g=!1,A=!1,T=Xe({path:t.path,headers:t.headers,onContent(f){g=!1,x.follow(),u.hide(),k+=f.text,t.renderer.render(k,a)},onToolCall(f){c.onToolCall(f),g=!1,x.follow()},onToolReturn(f){c.onToolReturn(f),g=!0,x.follow()},onGoodbye(f){c.active.clear(),g=!1,x.follow(),S.busy(!1),k.trim()===""&&f.answer&&t.renderer.render(f.answer,a),h()},onError(f){c.active.clear(),g=!1,x.follow(),S.busy(!1),u.hide(),a.classList.add("is-error");let R="Something went wrong. Please try again.";try{R=f.error||R}catch{}a.textContent=R,h()}});return e.talk=f=>{T.active()||(A=!0,c.active.clear(),c.completed.clear(),k="",a.innerHTML="",a.classList.remove("is-error"),u.hide(),S.busy(!0),g=!0,x.follow(),T.attach(f),S.clear(),S.focus())},e.restore=f=>{l.hide(),f?.last_message?u.restore(f.last_message):u.show()},e.reset=async()=>{T.close();try{await t.http.destroy()}catch{}a.innerHTML="",a.classList.remove("is-error"),u.forget(),u.show(),c.active.clear(),c.completed.clear(),g=!1,A=!1,x.follow(),S.busy(!1),S.focus(),h()},e.focus=()=>S.focus(),e.clear=()=>{a.innerHTML="",a.classList.remove("is-error")},s.addEventListener("submit",f=>{f.preventDefault();let R=S.value.trim();R&&e.talk(R)}),i.addEventListener("click",e.reset),o.addEventListener("click",()=>{let f=r.classList.toggle("is-expanded");n.toggleAttribute("expanded",f),o.setAttribute("aria-expanded",f?"true":"false"),o.title=f?"Collapse chat":"Expand chat",S.focus()}),x.follow(),h(),p.set(y()),e.setPanels=f=>{t.panels=f||[],p.set(y())},e}var Xt=`
<style>
  :host {
    display: block;
