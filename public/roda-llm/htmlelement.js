...
`,t=t.substring(k.length+1),c=!0),!c){let h=this.rules.other.nextBulletRegex(v),g=this.rules.other.hrRegex(v),f=this.rules.other.fencesBeginRegex(v),x=this.rules.other.headingBeginRegex(v),_=this.rules.other.htmlBeginRegex(v);for(;t;){let S=t.split(`
`,1)[0],L;if(k=S,this.options.pedantic?(k=k.replace(this.rules.other.listReplaceNesting,"  "),L=k):L=k.replace(this.rules.other.tabCharGlobal,"    "),f.test(k)||x.test(k)||_.test(k)||h.test(k)||g.test(k))break;if(L.search(this.rules.other.nonSpaceChar)>=v||!k.trim())m+=`
