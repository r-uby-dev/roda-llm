...
${p}`:p,a=a?`${a}
${g}`:g;let w=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(g,s,!0),this.lexer.state.top=w,n.length===0)break;let k=s.at(-1);if(k?.type==="code")break;if(k?.type==="blockquote"){let y=k,x=y.raw+`
`+n.join(`
`),u=this.blockquote(x);s[s.length-1]=u,r=r.substring(0,r.length-y.raw.length)+u.raw,a=a.substring(0,a.length-y.text.length)+u.text;break}else if(k?.type==="list"){let y=k,x=y.raw+`
`+n.join(`
`),u=this.list(x);s[s.length-1]=u,r=r.substring(0,r.length-k.raw.length)+u.raw,a=a.substring(0,a.length-y.raw.length)+u.raw,n=x.substring(s.at(-1).raw.length).split(`
