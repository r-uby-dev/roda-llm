...
`,1)[0],S;if(v=k,this.options.pedantic?(v=v.replace(this.rules.other.listReplaceNesting,"  "),S=v):S=v.replace(this.rules.other.tabCharGlobal,"    "),m.test(v)||g.test(v)||w.test(v)||d.test(v)||h.test(v))break;if(S.search(this.rules.other.nonSpaceChar)>=E||!v.trim())b+=`
`+S.slice(E);else{if(x||y.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||m.test(y)||g.test(y)||h.test(y))break;b+=`
`+v}!x&&!v.trim()&&(x=!0),p+=k+`
...
<style>
  ...
  /* The words are the host's own text face; only what
     is code-shaped stays mono. */
  .ac-panel-row-title {
    color: inherit;
    flex: none;
    font-size: 1em;
