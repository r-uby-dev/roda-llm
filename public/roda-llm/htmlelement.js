...
`,t=t.substring(y.length+1),c=!0),!c){let d=this.rules.other.nextBulletRegex(x),h=this.rules.other.hrRegex(x),f=this.rules.other.fencesBeginRegex(x),S=this.rules.other.headingBeginRegex(x),k=this.rules.other.htmlBeginRegex(x);for(;t;){let g=t.split(`
`,1)[0],A;if(y=g,this.options.pedantic?(y=y.replace(this.rules.other.listReplaceNesting,"  "),A=y):A=y.replace(this.rules.other.tabCharGlobal,"    "),f.test(y)||S.test(y)||k.test(y)||d.test(y)||h.test(y))break;if(A.search(this.rules.other.nonSpaceChar)>=x||!y.trim())b+=`
`+A.slice(x);else{if(v||w.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||f.test(w)||S.test(w)||h.test(w))break;b+=`
`+y}!v&&!y.trim()&&(v=!0),p+=g+`
