...
class AgentConsole extends HTMLElement {
   ...
   * webfonts load or the console is resized. Re-measuring while an
   * answer is on screen is skipped, so a long answer never stretches
   * the console.
   */
  observePlaceholder() {
    ...
    if ("ResizeObserver" in window) {
      ...
      this.observer = new ResizeObserver(() => {
        if (!this.hasAttribute("height") && answer.childNodes.length === 0)
          this.applyHeight()
      })
