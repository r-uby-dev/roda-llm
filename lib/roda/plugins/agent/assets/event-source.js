 ...
 *
 * The platform's EventSource only makes GET requests, so it cannot carry a
 * body - and the body is where a prompt belongs. This speaks the same
 * protocol on the wire and reads like the interface it stands in for:
 * addEventListener, close, readyState, an Event with a data string.
 *
 * Three differences, all of them POST's doing: the body is yours to set, the
 * response is checked before it is read as a stream, and a failure arrives as
 * an error event rather than as a silent reconnection.
 *
 *   const es = createEventSource({url: "agents/theo", body: {q: "hello"}})
...
export function createEventSource(options = {}) {
  ...
  /**
   * A plain object of fields is form-encoded; a string, URLSearchParams or
   * FormData is left alone for fetch to send as it is.
   */
  const payload = (() => {
    if (body == null)
      return undefined
  ...
  /**
   * Listen for the events the server names, in the writer's own spelling:
   * the agent endpoint sends onContent, onToolCall, onToolReturn, onGoodbye
   * and onError.
   */
  ...
  fetch(url, {
    ...
    /**
     * A refusal is not a stream: say why rather than read an empty body as an
     * empty answer.
     */
    if (!response.ok)
