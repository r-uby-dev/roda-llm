 ...
 */
  ...
  /**
   * What the body holds while a route is being fetched. A panel slow to
   * answer should say so rather than leave last time's markup sitting
   * there, which is indistinguishable from a panel that never loaded.
   */
  function spinner() {
 ...
 */
   ...
   */
    ...
    /* Refreshing a live panel replaces its markup.
       Capture open disclosures before replacement,
       then restore them in the new tree. */
    const open = [...body.querySelectorAll("details[open][name]")]
                 .map((details) => details.getAttribute("name"))
    body.innerHTML = markup || ""
    for (const name of open) {
      const details = [...body.querySelectorAll("details[name]")]
