...
/* A markdown render with prism support */
...
export function panels({ root, host }) {
  ...
  /* An agent's message is markdown, the same as its answers are,
    so the same renderer draws it: marked, then Prism over the
    fenced blocks. */
  const md = renderer()
   ...
   */
    ...
    for (const details of fresh) {
      ...
      for (const other of fresh)
        if (other !== details && sameGroup(other, details))
          other.open = false
    }
    ...
    for (const el of body.querySelectorAll("[data-markdown]"))
      md.render(el.textContent, el)
