 ...
 *
 * A panel is a label, a badge, and a card. The host says what the card holds,
 * one of three ways:
 *
 ...
 *
 * A child with slot="aside" goes in the label. Hover opens one panel at a
 * time, above its own label; leaving both closes it.
 */
 ...
 */
  ...
  /**
   * Open the panel marked `focus-on-turn-start`, when a turn starts. The
   * console does not know what a trace is called, so the markup says which
   * panel it means.
   */
  self.follow = () => {
