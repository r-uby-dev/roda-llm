 ...
 */
  ...
  function describe(details, all) {
    ...
    return { name, at: same.indexOf(details), of: same.length, label: labelOf(details) }
  }
   ...
   */
    ...
    /* Immediately, before the await. The card is already open - what is
       missing is the body, and a route with nothing to show yet should
       look like one that is being fetched. */
    if (!quiet) {
      note.hidden = true
      body.innerHTML = ""
      body.append(spinner())
    ...
    try {
      ...
      /* A refresh that fails leaves what is on screen alone: the panel is
         still the last thing that was true. */
      if (!quiet) body.innerHTML = ""
    ...
    /* Refreshing a live panel replaces its markup, and a panel being read
       should stay where it was: remember which disclosures are open and
       put them back. Every open disclosure is remembered, named or not,
       and remembered by *which* one it was rather than only by the name it
       groups under - a list of spans all sharing one name put back the
       first of them, which is the newest, so opening an older span and
       letting the panel refresh reopened the newest instead. */
    const all = [...body.querySelectorAll("details")]
    const opened = all.filter((details) => details.open)
    ...
    for (const want of opened) {
      ...
      /* Same shape as before: the one in the same place. A span arriving at
         the top moves every position down one, so fall back to the summary
         that reads the same. */
      const found = (same.length === want.of ? same[want.at] : null)
        || same.find((details) => labelOf(details) === want.label)
      if (!found) continue
      /* The markup arrives with one of a named group already open. A name
         opens one at a time, so the rest go before this one does. */
      if (want.name !== null)
        for (const other of same) if (other !== found) other.open = false
      found.open = true
    }
  }
