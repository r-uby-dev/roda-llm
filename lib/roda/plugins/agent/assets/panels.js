 ...
 */
   ...
   */
    ...
    /* A refresh while one is already on the wire: the pass in flight will
       land with the server's latest, so a second request for the same panel
       is only load. An open the reader asked for is never held back. */
    if (quiet && started !== applied) return
  ...
  /* Passes started, and the newest one that has written the body. A pass is
