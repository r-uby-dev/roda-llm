 ...
 */
  ...
  function place(label) {
    ...
    /* A sidebar goes beside the console - unless there is no beside, in
       which case it covers it. A phone has no second column. */
    if (side === "right" && window.innerWidth >= 720) {
      view.style.top = `${Math.round(outer.top)}px`
      view.style.left = `${Math.round(outer.right)}px`
      view.style.width = `${Math.round(outer.width)}px`
      view.style.height = `${Math.round(outer.height)}px`
      view.style.bottom = "auto"
      return
    }
    if (side === "right") {
      view.style.top = `${Math.round(outer.top)}px`
      view.style.left = `${Math.round(outer.left)}px`
      view.style.width = `${Math.round(outer.width)}px`
      view.style.height = `${Math.round(outer.height)}px`
      view.style.bottom = "auto"
      return
  ...
  function watch() {
    ...
    document.addEventListener("pointerdown", (event) => {
      ...
      if (!path.includes(view) && !path.includes(strip)) self.close()
    }, true)
