...
<style>
  ...
  /* The live line: which tool is running, or "Thinking…" while the turn
     waits on the model. It sits above the composer on the prompt's own
     margin, so it reads as the console talking to itself, and it is only
     on screen during a turn. */
  .ac-status {
    align-items: center;
    color: var(--ac-muted);
    display: none;
  ...
  .ac-status.is-active {
    ...
    border-radius: var(--ac-radius);
    display: flex;
    /* The enclosure ends in a point on its bottom edge, toward the
       composer, the way a chat client marks what it is doing right now.
       The tail sits outside the box, on its own layer, and the margin
       below leaves it room. */
    clip-path: polygon(0 0, 100% 0, 100% 100%, 1rem 100%,
                       0.5rem calc(100% + 0.35rem), 0.05rem 100%, 0 100%);
