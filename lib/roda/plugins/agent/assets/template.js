...
<style>
  ...
  /* The live line: which tool is running, "Thinking…" while the turn waits
     on the model, and "Idle" the rest of the time. It is always on screen -
     the bar it sits in never blinks out. */
  .ac-status {
    align-items: center;
    color: var(--ac-muted);
    display: flex;
  ...
  .ac-status .is-idle { color: var(--ac-muted); }
  ...
  /* The status bar: the count on the left, the live call beside it. It
     runs the console's full width and carries its own rules, so the row
     reads as a component rather than as a stray line, and the list below
     opens out of it. */
  .ac-trace-summary {
    align-items: center;
    background: var(--ac-surface);
  ...
  /* The count and its chevron, on the left of the bar - and always there,
     even at zero, so the live call beside it does not have to move when
  ...
  <div class="ac-trace" part="trace">
    <details class="ac-trace-details" part="trace-details">
      <summary class="ac-trace-summary" part="trace-summary">
        <span class="ac-trace-toggle" part="trace-toggle">
