...
<style>
  ...
  /* The live line: which tool is running, or "Thinking…" while the
     turn waits on the model. It is only on screen during a turn. */
  .ac-status {
    align-items: center;
    color: var(--ac-muted);
    display: none;
  ...
  /* Their status panel: a filled chip, so activity reads as state
  ...
  /* A trace of what the agent did, folded into a summary line above
     the composer: the summary carries the live state (which tool is
     running, or how many ran), and the list is a disclosure away.
     Native <details>, so opening it is not our JavaScript. */
  .ac-trace {
    color: var(--ac-muted);
    font-family: var(--ac-font-mono);
    font-size: 0.86em;
  ...
  /* The running tool sits under the summary, outside the disclosure, so
     it is visible whether or not the list is open, and it goes away when
     the last call returns. Its line is reserved either way: the trace
     sits above the answer, so a line that came and went would resize the
     tray and move the answer under it - which reads as a ripple while a
     turn streams. */
  .ac-trace .ac-status {
    display: flex;
    margin-top: 0.43em;
  ...
  .ac-trace-details.is-empty { visibility: hidden; }
  ...
  .ac-trace-details[open] .ac-icon-chevron { transform: rotate(90deg); }
...
<div class="ac-console" part="console">
  ...
  <div class="ac-trace" part="trace" hidden>
    <details class="ac-trace-details" part="trace-details">
      <summary class="ac-trace-summary" part="trace-summary">
        <svg class="ac-icon ac-icon-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M9 6l6 6-6 6"/></svg>
