...
<style>
  ...
  /* The live line sits above the summary, outside the disclosure, so it
     is visible whether or not the list is open, and it goes away when the
     last call returns. Its line is reserved either way: the trace sits
     above the answer, so a line that came and went would resize the tray
     and move the answer under it - which reads as a ripple while a turn
     streams. */
  .ac-trace .ac-status {
    display: flex;
    margin: 0 0 0.43em;
