...
/**
 * Panels: <agent-panel> children, plus whatever the host
 * set on the element. Same descriptors.
 */
  ...
  /**
   * The status line says one thing: what the console is
   * doing. That is the tool that is running, "Thinking…"
   * while the turn waits on the model, and nothing at all
   * in between - an idle bar does not need to say it is
   * idle. It is always on screen, so the bar never blinks
   * out between one moment of a turn and the next.
   */
  const status = {
  ...
  const placeholder = {
    ...
    /**
     * Open on the last thing that was said. It takes the
     * invitation's place in the column rather than covering
     * it, and is rendered the way an answer is, because it
     * was one: the console keeps the height the invitation
     * gave it, and the body scrolls the message the way it
     * scrolls an answer.
     */
    restore(message) {
      if (!this.last)
  ...
  /**
   * The console asks where the conversation got to before
   * it can say anything, so it opens saying that, and the
   * answer to the question is either the invitation the
   * host slotted or the last thing that was said. Both go
   * through restore(): the element resolves the loading
   * state whether the app answered, refused or was never
   * asked.
   */
  const loading = {
...
/**
 * Where the conversation got to: asked on open and after
 * each turn, since a turn moves it on.
 */
  const settle = (data) => {
    loading.hide()
/**
 * The describe answer, announced for whoever wants it: what
 * it means is their business.
 */
  ...
  /*
   * Icons the console falls back to when the host slots
   * none of its own, so a tool call and a tool return are
   * still legible.
   */
  const icons = {
    el: root,
  ...
  const input = {
    ...
    busy(value) {
       ...
       * loses focus, so a turn would drop the caret the
       * user just typed with.
       */
      this.el.readOnly = value
      /*
       * What the prompt reads to know a turn is running,
       * and what the page can read from the host.
       */
      ...
      if (value)
        panels.follow()
      /*
       * The field says what is happening while it waits,
       * and goes back to what the host asked for when the
       * turn is over.
       */
      if (value) {
    ...
    focus() {
      /*
       * Never scroll: focus follows the conversation, and
       * the page should hold still while it does.
       */
  ...
  let waiting = false
  /**
   * Untouched until the first turn: describe may put the
   * last message where the invitation was.
   */
  let started = false
  ...
  self.talk = (q) => {
    ...
    waiting = true
      /*
       * A turn reads as one turn: the answer and the live
       * line start empty.
       */
    status.follow()
    stream.attach(q)
  ...
  /**
   * The describe request has come back: show what it
   * reported, or the host's invitation when there was
   * nothing to report. Either way the console stops saying
   * it is loading.
   */
  self.restore = (data) => {
    loading.hide()
  ...
  expand.addEventListener("click", () => {
    ...
    /**
     * Mirror the state onto the host, so the page that owns
     * the console can react to it - an expanded console
     * usually wants to leave the page's flow and take the
     * viewport.
     */
  ...
  /*
   * The bar is on screen from the start, so the console
   * opens on its resting state rather than on an empty
   * line.
   */
  status.follow()

  /**
   * Where the conversation got to: the invitation, or the
   * last message.
   */
  refresh()
  ...
  /**
   * The host's panels, for a console already on the page. A
   * console with <agent-panel> children needs none of this:
   * same descriptors, and the two compose.
   */
