...
export function console(options = {}) {
  ...
  const declared = () =>
    (slot?.assignedElements() || [])
      ...
      .map((el) => el.panel())
  const collect = () =>
    [...declared(), ...(options.panels || []), memory.panel()].filter(Boolean)
  if (slot)
   ...
   *
   * Memory is a panel like any other: it puts the meter in its label and the
   * sentence in its card, and the panel does the rest - opening on hover,
   * placing itself above the label, fetching, closing. One implementation of a
   * card, rather than two that have to be kept in step.
   */
  const memory = (() => {
    const source = root.querySelector("template.ac-memory")
    if (!source) return {render() {}, panel: () => null}
    const fragment = source.content.cloneNode(true)
    const meter = fragment.querySelector(".ac-memory-meter")
    const figures = fragment.querySelector(".ac-memory-figures")
    return {
      meter,
      fill: meter?.querySelector(".ac-memory-fill"),
      value: meter?.querySelector(".ac-memory-value"),
      figure: figures?.querySelector(".ac-memory-figure"),
      /**
       * Memory, as the panel it is: the meter in the label, the sentence in
       * the card.
       * @return [Hash]
       */
      panel: () => ({
        label: "Memory",
        aside: meter,
        render: (body) => body.append(figures)
      }),
      render(data) {
        ...
        // The sentence behind the case is written in the template; the one
        // thing put in it here is the number, since the words around it are
        // the console's and not its own.
        if (this.figure)
          this.figure.textContent = percent
        if (this.meter) {
          this.meter.setAttribute("aria-valuenow", String(percent))
          // The two bands are the console raising its voice: more than half
          // the context spent, and nearly all of it.
