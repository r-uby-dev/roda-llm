...
export function console(options = {}) {
  ...
  /**
   * The tray holds the trace and the status line, and comes and goes as
   * a whole: one place to look for what the agent is doing.
   */
  /**
   * The tray holds the trace and the live call, and comes and goes as a
   * whole: it is on screen for the length of a turn - the live call is
   * there from the first keystroke of the turn, before any tool has run -
   * and stays after the turn while it has a trace to show.
   */
  const tray = {
    el: root.querySelector(".ac-trace"),
    sync() {
      this.el.hidden = actions.entries.size === 0 && !status.el.classList.contains("is-active")
    }
  }
  ...
  const status = {
    ...
    /**
     * One line, three states: the tool that is running, "Thinking…"
     * while the turn waits on the model without a tool in flight, and
     * nothing at all when the turn is over.
     */
    follow() {
  ...
  const actions = {
    ...
    summarise() {
      ...
      if (this.details)
        this.details.classList.toggle("is-empty", count === 0)
      tray.sync()
    },
    call(tool) {
