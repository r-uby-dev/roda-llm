...
export function console(options = {}) {
  ...
  const separator = root.querySelector(".ac-trace-separator")
  ...
  const status = {
    ...
    render({ text, icon, thinking }) {
      ...
      // The dot in front of the count only belongs there when the live
      // line has something to say.
      if (separator)
        separator.hidden = el.children.length === 0
    },
    follow() {
