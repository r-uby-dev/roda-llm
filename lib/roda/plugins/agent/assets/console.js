...
export function console(options = {}) {
  ...
  const memory = {
    ...
    figures: root.querySelector(".ac-memory-help-figures"),
    render(data) {
      ...
      // The card behind the case says the same thing in figures, since a
      // percentage is a summary and this is the thing being summarised.
      if (this.figures)
        this.figures.textContent = available > 0
          ? `${used.toLocaleString()} of ${available.toLocaleString()}`
          : "Nothing known to be spent yet"
      if (this.meter) {
