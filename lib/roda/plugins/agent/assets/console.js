...
export function console(options = {}) {
  ...
  /**
   * The status line says one thing: what the console is doing. It has
   * three states - the tool that is running, "Thinking…" while the turn
   * waits on the model, and "Idle" the rest of the time - and it is
   * always on screen, so the bar at the foot of the console never blinks
  ...
  const status = {
    ...
    thinking: "Thinking…",
    idle: "Idle",
    render({ text, icon, thinking, idle }) {
      const { el } = this
      el.replaceChildren()
      if (icon)
    ...
    follow() {
      ...
      this.render({text: this.idle, idle: true})
    }
  }
  ...
  const actions = {
    ...
    toggle: root.querySelector(".ac-trace-toggle"),
    ...
    summarise() {
      ...
      if (this.toggle)
        this.toggle.classList.toggle("is-empty", count === 0)
    },
    call(tool) {
  ...
  expand.addEventListener("click", () => {    const expanded = root.classList.toggle("is-expanded")
    /**
