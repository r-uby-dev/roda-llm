...
/**
 * The console's end of a turn: one stream per turn,
 * addressed by the agent's path, with the prompt in the
 * body. The events are the ones the agent endpoint sends,
 * so this is a thin naming of them over the
 * createEventSource interface - the console gets
 * `onContent`, `onToolCall`, `onToolReturn`, `onGoodbye`
 * and `onError`, and a failed request is reported as an
