...
class AgentConsole extends HTMLElement {
   ...
   */
  applyPrompt(value) {
    // The sign, not the prompt: the prompt also holds the mark that
    // replaces it while a turn runs, and writing here would wipe it.
 ...
 *
 * `src` says where the contents come from, fetched
 ...
 * slot="aside" goes in the label, the rest in the
 * card.
 */
class AgentPanel extends HTMLElement {
  ...
  panel() {
    ...
    return {
      ...
      /* `html` says the route answers with the panel's markup rather than with
         rows: the page draws it, and this only puts it in the card. */
      html: this.hasAttribute("html"),
      load: async () => {
        if (!this.src) return this.hasAttribute("html") ? "" : []
        const res = await fetch(this.src)
        return this.hasAttribute("html") ? await res.text() : await res.json()
      }
    }
  }
}
