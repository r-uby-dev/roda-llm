...
class AgentConsole extends HTMLElement {
  ...
  /**
   * Ask the app where this conversation got to, and open on that. It is
   * the same describe route the plugin serves, so a host without a
   * session, a token or that route at all simply keeps the placeholder
   * it slotted in: nothing about the console depends on the answer.
   */
  async restore(client) {
    try {
      ...
      if (!response.ok)
        return
      this.console.restore(await response.json())
    } catch {}
  }
