...
class AgentConsole extends HTMLElement {
  ...
  connectedCallback() {
    ...
    /* A panel that asks to be open is open when the console
       arrives. `show` and not a click: it is idempotent, so a
       panel the reader already opened is left alone. */
    const open = this.panels.find((panel) => panel.open)
    if (open && !this.console.open)
      this.console.show(open.label)
